...
public class CMaps implements CMapService {
	...
	// --- Kiểm tra tồn tại cmap id trong danh sách cmaps
	private boolean contain(int cmap_id, Set<CMap> cmaps) {
		for (CMap c : cmaps) {
			if (c.getId() == cmap_id)
				return true;
		}
	...
	public int create(CMapDetail cmap, String username) {
		...
		try {
			...
			CMap entityCMap = new CMap(cmap.getTitle(), cmap.getInfo(), author);
			...
			for (ConceptVM conceptvm : listNode) {
				...
				Concept temp = new Concept(conceptvm.getText(), conceptvm.getLoc(),
						entityCMap);
				concepts.save(temp);
			...
			for (RelationVM relation : listLink) {
				...
				Relation temp = new Relation(from, to, relation.getText(),
						entityCMap);
				relations.save(temp);
			}
	...
	public ShareVM getShare(int cmap_id) {
		...
		// --- Tạo đối tượng ShareVM mới chứa danh sách member chưa share và đã
		// share
		...
		for (Member m : mems.findAll()) {
			...
			MemberVM temp = new MemberVM(m.getId(), m.getFullname(),
					...
					m.getUsername());
			// --- nếu member chứa cmap id trên thì thêm vào danh sách đã share
			if (contain(cmap_id, m.getCmaps())) {
