...
public class Member {
	...
	@JoinTable(name = "shared", joinColumns = { @JoinColumn(name = "member_id") }, inverseJoinColumns = { @JoinColumn(name = "cmap_id", nullable = false, updatable = false, insertable = false) })
	private Set<CMap> shares = new HashSet<CMap>(0);
	// --- Danh sách bài tập được gán
	@ManyToMany(cascade = CascadeType.ALL, fetch = FetchType.EAGER)
	@JoinTable(name = "assign_member", joinColumns = { @JoinColumn(name = "member_id") }, inverseJoinColumns = { @JoinColumn(name = "assign_id", nullable = false, updatable = false, insertable = false) })
	...
	public CMap findInShare(int cmap_id) {
		...
		for (CMap c : this.shares) {
			// --- nếu cmap này trùng id với cmap_id thì xóa
			if (c.getId() == cmap_id) {
				return c;
			}
		}
